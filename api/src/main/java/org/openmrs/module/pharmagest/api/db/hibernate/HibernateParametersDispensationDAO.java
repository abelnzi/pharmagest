...
public class HibernateParametersDispensationDAO implements
	...
	public Produit getProduitById(Integer produitId) {
		...
		return (Produit) getSessionFactory().getCurrentSession().get(
				Produit.class, produitId);
	}

	public Regime getRegimeById(Integer regimeId) {
		// TODO Auto-generated method stub
		return (Regime) getSessionFactory().getCurrentSession().get(
				Regime.class, regimeId);
	}

	public Medecin getMedecinById(Integer medecinId) {
		// TODO Auto-generated method stub
		return (Medecin) getSessionFactory().getCurrentSession().get(
				Medecin.class, medecinId);
	}

	@Override
	public Programme getProgrammeById(Integer programmeId) {
		return (Programme) getSessionFactory().getCurrentSession().get(
				Programme.class, programmeId);
	}

	@Override
	public Fournisseur getFournisseurById(Integer fournisseurId) {
		return (Fournisseur) getSessionFactory().getCurrentSession().get(
				Fournisseur.class, fournisseurId);
	}

	@SuppressWarnings("unchecked")
	@Override
	public Collection<Fournisseur> getAllFournisseurs() {
		return (Collection<Fournisseur>) getSessionFactory().getCurrentSession()
				.createCriteria(Fournisseur.class).list();
	}

	@SuppressWarnings("unchecked")
	@Override
	public Collection<Programme> getAllProgrammes() {
		return (Collection<Programme>) getSessionFactory().getCurrentSession()
				.createCriteria(Programme.class).list();
	}

	@SuppressWarnings("unchecked")
	@Override
	public Collection<TypeOperation> getAllTypeOperation() {
		return (Collection<TypeOperation>) getSessionFactory().getCurrentSession()
				.createCriteria(TypeOperation.class).list();
	}

	@Override
	public TypeOperation getTypeOperationById(Integer typeOperationId) {
		return (TypeOperation) getSessionFactory().getCurrentSession().get(
				TypeOperation.class, typeOperationId);
	}

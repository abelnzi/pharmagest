...
<form:form method="post"
	action="${pageContext.request.contextPath}/module/pharmagest/listReceptionModif.form"
...
<c:if test="${suppr =='suppr'}">
		<div id="openmrs_msg">la reception du fournisseur ${fournisseur} et du programme ${programme} à la date du ${dateRecept} à été supprimée avec succès</div>
</c:if>
<br>
<br>
<div>
           ...
           <tbody>
                ...
                <c:forEach var="lo" items="${receptions}">
								<tr>
									...
									<td width="21%"><a href="<c:url value="/module/pharmagest/modifierReception.form"><c:param name="paramId" 
                                                  value="${lo.recptId}"/></c:url>">Choix</a>
                                                  <a href="<c:url value="/module/pharmagest/listReceptionModif.form">
									  <c:param name="deleteId" value="${lo.recptId}"/> </c:url>" 
                                      onClick="return confirm('Voulez vous supprimer?') ? true : false;" >Supprimer</a>
                                    </td>
								</tr>

...
public class SimulationCommandeController {

	@RequestMapping(value = "/module/pharmagest/simulationCommande.form", method = RequestMethod.GET)
	public String initRapportConso(ModelMap model) {
		...
		return "/module/pharmagest/simulationCommande";
	}

	@RequestMapping(value = "/module/pharmagest/simulationCommande.form", method = RequestMethod.POST, params = {
			"btn_rechercher" })
			...
			if (!(commandes.isEmpty())) {
				...
				arrayProg.add(3);arrayProg.add(4);arrayProg.add(8);		
					...
					for (RapportCommandeBean conso : listConso.values()) {
						...
						if(sdu2!=0 && sdu3!=0 )conso.setCmm((int) Math.round((conso.getQteUtil()+sdu2+sdu3)/3));
						if(conso.getCmm()!=0)conso.setMoisStock(this.arrondir(((double) conso.getStockDispo()/conso.getCmm()),1));

...
public class TraitementsPPSController {
	...
	public void valider(@ModelAttribute("formulaireTraitementsPPS") FormulaireTraitementsPPS formulaireTraitementsPPS,
		...
		if (!result.hasErrors()) {
			...
			/************************* Verification du stock de produits****************************/
			...
			while(boucle && i <= taille){
					...
					if (!stockerList.isEmpty()) {
						...
						if (qteStock < qte) { //verifier si la quantité en stock n'est pas suffisante
							boucle=false;
			...
			if(boucle==true){// si tous les produits ont du stock suffisant
				...
				int j = 1;
				int size = formulaireTraitementsPPS.getTabSaisiesPPS().getLigneCommandeSitesCollection().size();
				
				while( j <= size){
				
					String idProd = request.getParameter("idProd" + j);
					String qtePro = request.getParameter("qtePro" + j);
					int proID=(idProd!=null)?Integer.parseInt(idProd):0;
			...
			} else {// si un produit n'a pas de stock suffisant

...
public class DispensationController {
	...
	public void rechercher(
			@ModelAttribute("formulaireOrdonnance") FormulaireOrdonnance formulaireOrdonnance,
		...
		PatientIdentifier patientIdentifier = Context.getPatientService()
				.getPatientIdentifier(numPatient);
		if (patientIdentifier != null) {
			patientIdentifier.getPatient().getPatientIdentifier()
					.getIdentifier();
			model.addAttribute("patientIdentifier",
					patientIdentifier.getPatient());
			List<Regime> regimes = (List<Regime>) Context.getService(
					ParametersDispensationService.class).getAllRegimes();
			List<Produit> produits = (List<Produit>) Context.getService(
					ParametersDispensationService.class).getAllProduits();
			List<Medecin> medecins = (List<Medecin>) Context.getService(
					ParametersDispensationService.class).getAllMedecins();
		...
		try {
			...
			if (!result.hasErrors()) {
				...
				if (formulaireOrdonnance.getProduit() != null)
					stockerId.setProdId(formulaireOrdonnance.getProduit()
							.getProdId());
				if (formulaireOrdonnance.getProgramme() != null)
					stockerId.setProgramId(formulaireOrdonnance.getProgramme()
							.getProgramId());
				...
				if (stockQte >= formulaireOrdonnance.getServQteServi()) {
					...
					lgnDisp.setServPrixUnit(formulaireOrdonnance
							.getServPrixUnit());
					lgnDisp.setServQteDem(formulaireOrdonnance.getServQteDem());
					lgnDisp.setServQteServi(formulaireOrdonnance
							.getServQteServi());
					lgnDisp.setId(formulaireOrdonnance.addLigneDispensationId());
					formulaireOrdonnance.getTabdispensation()
							.addLigneDispensation(lgnDisp);
	...
	public void saveDispensation(
		...
		if (!result.hasErrors()) {
			...
			Map lignes = formulaireOrdonnance.getTabdispensation()
					.getLigneDispensations();
			for (Iterator i = lignes.keySet().iterator(); i.hasNext();) {
				...
				Context.getService(DispensationService.class)
						.saveLigneDispensation(ld);
				// gestion du Stocker
				StockerId stockerId = new StockerId();
				stockerId.setProdId(ligne.getProduit().getProdId());
				stockerId.setProgramId(formulaireOrdonnance.getOrdonnance()
						.getProgramme().getProgramId());
				Stocker stocker = Context.getService(GestionStockService.class)
						.getStockerById(stockerId);
				if (stocker != null) {
	...
	public void deleteLigneDispensation(
		...
		model.addAttribute("medecins", medecins);
		model.addAttribute("regimes", regimes);
		formulaireOrdonnance.getTabdispensation().removeLigneDispensationById(
				paramId);
		model.addAttribute("ligneDispensations", formulaireOrdonnance
				.getTabdispensation().getLigneDispensationsCollection());
		model.addAttribute("var", "1");
	...
	public void initBinder(WebDataBinder binder) {
		...
		binder.registerCustomEditor(Regime.class, new PropertyEditorSupport() {
			@Override
			public void setAsText(String text) throws IllegalArgumentException {
				Regime regime = Context.getService(
						ParametersDispensationService.class).getRegimeById(
		...
		binder.registerCustomEditor(Medecin.class, new PropertyEditorSupport() {
			@Override
			public void setAsText(String text) throws IllegalArgumentException {
				Medecin medecin = Context.getService(
						ParametersDispensationService.class).getMedecinById(
						Integer.parseInt(text));
